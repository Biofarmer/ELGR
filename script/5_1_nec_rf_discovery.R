...
#subtype
train_st <- list()
train_st[[1]] <- c("MasiAC_2021","RahmanSF_2018","RavehSadkaT_2016")
train_st[[2]] <- c("BrooksB_2017","RahmanSF_2018","RavehSadkaT_2016")
train_st[[3]] <- c("BrooksB_2017","MasiAC_2021","RavehSadkaT_2016")
train_st[[4]] <- c("BrooksB_2017","MasiAC_2021","RahmanSF_2018")
test_st <- list()
test_st[[1]] <- "BrooksB_2017"
test_st[[2]] <- "MasiAC_2021"
test_st[[3]] <- "RahmanSF_2018"
test_st[[4]] <- "RavehSadkaT_2016"

rf_auc_loso_subtype_047 <- data.frame("study"="x","auc"=0,"mtry"=0,stringsAsFactors = F)[-1,]
for (i in 1:length(train_st)) {
   print(i)
   train_set <- window_db_sel_subtype_meta %>% filter(Study%in%train_st[[i]])%>%dplyr::select(-Study)
   x_train <- train_set%>%dplyr::select(-nec)%>%column_to_rownames("SequenceID")%>%dplyr::select(sort(names(.)))
   y_train <- as.factor(train_set$nec)
   for (m in c(seq(1,ncol(x_train),1))) {
      set.seed(1000)
      rf_train <- randomForest(x=x_train,y=y_train, importance = TRUE, ntree = 500, proximity = TRUE,mtry=m)
      test_set <- window_db_sel_subtype_meta %>% filter(Study==test_st[[i]])%>%dplyr::select(-Study)
      x_test <- test_set%>%dplyr::select(-nec)%>%column_to_rownames("SequenceID")%>%dplyr::select(sort(names(.)))
      y_test <- test_set$nec
      rf_pred_prob <- predict(rf_train, newdata = x_test, type = "prob")
      rf_pred <- predict(rf_train, newdata = x_test)%>%as.data.frame()
      ...
      pred <- ROCR::prediction(comb[,3], comb[,1])
      auc <- ROCR::performance(pred,"auc")@y.values[[1]]
      rf_auc_loso_subtype_047[nrow(rf_auc_loso_subtype_047)+1,] <- c(test_st[[i]],auc,m)
   }
}
#species
rf_auc_loso_species_047 <- data.frame("study"="x","auc"=0,"mtry"=0,stringsAsFactors = F)[-1,]
for (i in 1:length(train_st)) {
   print(i)
   train_set <- window_db_sel_species_meta %>% filter(Study%in%train_st[[i]])%>%dplyr::select(-Study)
   x_train <- train_set%>%dplyr::select(-nec)%>%column_to_rownames("SequenceID")%>%dplyr::select(sort(names(.)))
   y_train <- as.factor(train_set$nec)
   for (m in c(seq(1,ncol(x_train),1))) {
      set.seed(1000)
      rf_train <- randomForest(x=x_train,y=y_train, importance = TRUE, ntree = 500, proximity = TRUE,mtry=m)
      test_set <- window_db_sel_species_meta %>% filter(Study==test_st[[i]])%>%dplyr::select(-Study)
      x_test <- test_set%>%dplyr::select(-nec)%>%column_to_rownames("SequenceID")%>%dplyr::select(sort(names(.)))
      y_test <- test_set$nec
      rf_pred_prob <- predict(rf_train, newdata = x_test, type = "prob")
      rf_pred <- predict(rf_train, newdata = x_test)%>%as.data.frame()
      ...
      pred <- ROCR::prediction(comb[,3], comb[,1])
      auc <- ROCR::performance(pred,"auc")@y.values[[1]]
      rf_auc_loso_species_047[nrow(rf_auc_loso_species_047)+1,] <- c(test_st[[i]],auc,m)
   }
}
#subtype+species
rf_auc_loso_subtype_species_047 <- data.frame("study"="x","auc"=0,"mtry"=0,stringsAsFactors = F)[-1,]
for (i in 1:length(train_st)) {
   print(i)
   train_set <- window_db_sel_subtype_species_meta %>% filter(Study%in%train_st[[i]])%>%dplyr::select(-Study)
   x_train <- train_set%>%dplyr::select(-nec)%>%column_to_rownames("SequenceID")%>%dplyr::select(sort(names(.)))
   y_train <- as.factor(train_set$nec)
   for (m in c(seq(1,ncol(x_train),1))) {
      set.seed(1000)
      rf_train <- randomForest(x=x_train,y=y_train, importance = TRUE, ntree = 500, proximity = TRUE,mtry=m)
      test_set <- window_db_sel_subtype_species_meta %>% filter(Study==test_st[[i]])%>%dplyr::select(-Study)
      x_test <- test_set%>%dplyr::select(-nec)%>%column_to_rownames("SequenceID")%>%dplyr::select(sort(names(.)))
      y_test <- test_set$nec
      rf_pred_prob <- predict(rf_train, newdata = x_test, type = "prob")
      rf_pred <- predict(rf_train, newdata = x_test)%>%as.data.frame()
      ...
      pred <- ROCR::prediction(comb[,3], comb[,1])
      auc <- ROCR::performance(pred,"auc")@y.values[[1]]
      rf_auc_loso_subtype_species_047[nrow(rf_auc_loso_subtype_species_047)+1,] <- c(test_st[[i]],auc,m)
   }
}
####################################################################################################################################
#Preditive performance based on batch adjusted features:
#featues: subtype, species, subtype+species 
#LOSO
####################################################################################################################################
#subtype
window_db_subtype <- read.csv("data/window_db_subtype_sel_adj.csv",check.names=FALSE,stringsAsFactors = FALSE)
window_db_subtype <- window_db_subtype%>%column_to_rownames("subtype")%>%t()%>%as.data.frame()%>%rownames_to_column("SequenceID")
#species
metaphlan4_species <- read.csv("data/metaphlan4_species_sel_adj.csv",check.names=FALSE,stringsAsFactors = FALSE)
metaphlan4_species <- metaphlan4_species%>%column_to_rownames("species")
#subtype
window_db_subtype_rel <- window_db_subtype
window_db_subtype_047_adj <- window_db_subtype_rel%>%filter(SequenceID%in%window_db_047_adj$SequenceID)%>%column_to_rownames("SequenceID")
window_db_subtype_047_adj <- window_db_subtype_047_adj[,which(colSums(window_db_subtype_047_adj)!=0)]
window_db_subtype_047_adj <- window_db_subtype_047_adj%>%rownames_to_column("SequenceID")
window_db_sel_subtype_meta <- right_join(metadata_prenec%>%dplyr::select(SequenceID,Study,nec),window_db_subtype_047_adj,by="SequenceID")
window_db_sel_subtype_meta <- window_db_sel_subtype_meta%>%filter(Study%in%c("BrooksB_2017","MasiAC_2021","RahmanSF_2018","RavehSadkaT_2016"))%>%
   dplyr::select(1:3,which(colSums(.[-c(1:3)])!=0)+3)
...
#species
metaphlan4_species_t <- metaphlan4_species%>%t()%>%as.data.frame()%>%rownames_to_column("SequenceID")
window_db_species_047_adj <- metaphlan4_species_t%>%filter(SequenceID%in%window_db_047_adj$SequenceID)%>%column_to_rownames("SequenceID")
window_db_species_047_adj <- window_db_species_047_adj[,which(colSums(window_db_species_047_adj)!=0)]
window_db_species_047_adj <- window_db_species_047_adj%>%rownames_to_column("SequenceID")
window_db_sel_species_meta <- right_join(metadata_prenec%>%dplyr::select(SequenceID,Study,nec),window_db_species_047_adj,by="SequenceID")
window_db_sel_species_meta <- window_db_sel_species_meta%>%filter(Study%in%c("BrooksB_2017","MasiAC_2021","RahmanSF_2018","RavehSadkaT_2016"))%>%
   dplyr::select(1:3,which(colSums(.[-c(1:3)])!=0)+3)

#subtype + species
window_db_sel_subtype_species_meta <- left_join(window_db_sel_subtype_meta,window_db_species_047_adj%>%dplyr::select(SequenceID,names(window_db_sel_species_meta)[-c(1:3)]),by="SequenceID")
...
rf_auc_loso_subtype_047_adj <- data.frame("study"="x","auc"=0,"mtry"=0,stringsAsFactors = F)[-1,]
for (i in 1:length(train_st)) {
   print(i)
   train_set <- window_db_sel_subtype_meta %>% filter(Study%in%train_st[[i]])%>%dplyr::select(-Study)
   x_train <- train_set%>%dplyr::select(-nec)%>%column_to_rownames("SequenceID")%>%dplyr::select(sort(names(.)))
   ...
   for (m in c(seq(1,ncol(x_train),1))) {
      ...
      pred <- ROCR::prediction(comb[,3], comb[,1])
      auc <- ROCR::performance(pred,"auc")@y.values[[1]]
      rf_auc_loso_subtype_047_adj[nrow(rf_auc_loso_subtype_047_adj)+1,] <- c(test_st[[i]],auc,m)
   }
}
#species
rf_auc_loso_species_047_adj <- data.frame("study"="x","auc"=0,"mtry"=0,stringsAsFactors = F)[-1,]
for (i in 1:length(train_st)) {
   print(i)
   train_set <- window_db_sel_species_meta %>% filter(Study%in%train_st[[i]])%>%dplyr::select(-Study)
   x_train <- train_set%>%dplyr::select(-nec)%>%column_to_rownames("SequenceID")%>%dplyr::select(sort(names(.)))
   ...
   for (m in c(seq(1,ncol(x_train),1))) {
      ...
      pred <- ROCR::prediction(comb[,3], comb[,1])
      auc <- ROCR::performance(pred,"auc")@y.values[[1]]
      rf_auc_loso_species_047_adj[nrow(rf_auc_loso_species_047_adj)+1,] <- c(test_st[[i]],auc,m)
   }
}
#subtype + species
rf_auc_loso_subtype_species_047_adj <- data.frame("study"="x","auc"=0,"mtry"=0,stringsAsFactors = F)[-1,]
for (i in 1:length(train_st)) {
   print(i)
   train_set <- window_db_sel_subtype_species_meta %>% filter(Study%in%train_st[[i]])%>%dplyr::select(-Study)
   x_train <- train_set%>%dplyr::select(-nec)%>%column_to_rownames("SequenceID")%>%dplyr::select(sort(names(.)))
   ...
   for (m in c(seq(1,ncol(x_train),1))) {
      ...
      pred <- ROCR::prediction(comb[,3], comb[,1])
      auc <- ROCR::performance(pred,"auc")@y.values[[1]]
      rf_auc_loso_subtype_species_047_adj[nrow(rf_auc_loso_subtype_species_047_adj)+1,] <- c(test_st[[i]],auc,m)
   }
}
