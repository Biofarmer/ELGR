...
term_type_sum_richness <- term_type%>%column_to_rownames("names")%>%
...
term_type_sum_abund <- left_join(term_type_sum_abund,metadata_cb%>%dplyr::select(SequenceID,Study,st_subjectID),by=c("sample"="SequenceID"))
...
term_type_p <- ggplot() +
   ...
   geom_line(data=term_type_sum_se, aes(x=type, y=prevalence/1.5,group=1),linetype = "dashed",color="#E09137",size=0.5)+
   ...
   scale_y_continuous(expand = c(0,0),name= "Mean ARG abundance (capc)",limits = c(0,0.8),
...
preterm_type_sum_abund <- preterm_type%>%column_to_rownames("names")%>%
   t()%>%as.data.frame()%>%rownames_to_column("sample")%>%dplyr::summarise("sample"=sample,"type_total_abund"=rowSums(.[-1]))
preterm_type_sum_abund <- left_join(preterm_type_sum_abund,metadata_cb%>%dplyr::select(SequenceID,Study,st_subjectID),by=c("sample"="SequenceID"))
...
total_fit <- lmerTest::lmer(type_total_abund ~ group+(1|Study)+(1|st_subjectID),data =preterm_type_sum_abund_term)
anova(total_fit)
term_preterm_type_pval <- data.frame("type"="x","term_mean"=0,"term_se"=0,"term_prevalence"=0,"term_median"=0,"preterm_mean"=0,"preterm_se"=0,"preterm_prevalence"=0,"preterm_median"=0,"pval_wil"=0,"pval_lm"=0,stringsAsFactors = F)[-1,]
...
for (t in type_list) {
   ...
   db_cb <- left_join(db_cb,metadata_cb%>%dplyr::select(SequenceID,Study,st_subjectID),by="SequenceID")
   db_cb$Study <- as.factor(db_cb$Study)
   db_cb$group <- as.factor(db_cb$group)
   pval_wil <- pvalue(wilcox_test(type ~ group|Study, data=db_cb))
   lm_fit <- lmerTest::lmer(type ~ group+(1|Study)+(1|st_subjectID),data = db_cb)
   pval_lm <- anova(lm_fit)
   term_preterm_type_pval[nrow(term_preterm_type_pval)+1,] <- c(t,mean(db_term$type),std.error(db_term$type),sum(db_term$type>0)/1619,median(db_term$type),
                                                                mean(db_preterm$type),std.error(db_preterm$type),sum(db_preterm$type>0)/5684,median(db_preterm$type),
                                                                pval_wil,pval_lm$`Pr(>F)`)
}
term_preterm_type_pval <- term_preterm_type_pval%>%mutate_at(vars(-type),as.numeric)
term_preterm_type_pval$fdr <- p.adjust(term_preterm_type_pval$pval_lm,method = "fdr")
term_preterm_type_pval_sel <- term_preterm_type_pval%>%filter(fdr<0.05)%>%
   mutate(diff=preterm_mean-term_mean)%>%arrange(desc(preterm_mean))
term_type_g <- term_type%>%gather(.,sample,value,-names)%>%mutate(group="full_term")%>%filter(names%in%term_preterm_type_pval_sel$type)
preterm_type_g <- preterm_type%>%gather(.,sample,value,-names)%>%mutate(group="preterm")%>%filter(names%in%term_preterm_type_pval_sel$type)
term_preterm_type_sum <- rbind(term_type_g,preterm_type_g)
term_preterm_type_sum$names <- factor(term_preterm_type_sum$names,levels = term_preterm_type_pval_sel$type)
term_preterm_type_sum$group <- factor(term_preterm_type_sum$group,levels = c("preterm","full_term"))
#Figure 2a
term_preterm_type_p <- ggplot(term_preterm_type_sum, aes(x=names, y=value,fill=group)) +
   geom_boxplot(outliers = F)+
   scale_fill_manual(values = c("#4994C4","#E09137"),labels=c("preterm"="Preterm","full_term"="Term"))+
   scale_y_continuous(name= "ARG types abundance (capc)")+
   theme_bw()+
...
#alpha diversity of ARG subtype
...
term_subtype_abund <- term_subtype_t%>%rownames_to_column("sample")%>%dplyr::summarise("SequenceID"=sample,"subtype_total_abund"=rowSums(.[-1]))
term_subtype_richness <- rowSums(term_subtype_t != 0)
term_subtype_shannon <- diversity(term_subtype_t, index = "shannon", MARGIN=1) 
term_subtype_simpson <- diversity(term_subtype_t, index = "simpson", MARGIN=1) 
...
term_subtype_alpha <- left_join(term_subtype_alpha,term_subtype_abund,by="SequenceID")
...
preterm_subtype_abund <- preterm_subtype_t%>%rownames_to_column("sample")%>%dplyr::summarise("SequenceID"=sample,"subtype_total_abund"=rowSums(.[-1]))
...
preterm_subtype_simpson <- diversity(preterm_subtype_t, index = "simpson", MARGIN=1) 
...
preterm_subtype_alpha <- left_join(preterm_subtype_alpha,preterm_subtype_abund,by="SequenceID")
...
anova(lmerTest::lmer(Richness ~ Term+(1|Study)+(1|st_subjectID),data =subtype_alpha))
anova(lmerTest::lmer(Shannon ~ Term+(1|Study)+(1|st_subjectID),data =subtype_alpha))
subtype_alpha_month <- subtype_alpha%>%filter(!is.na(DOL))
subtype_alpha_month$month <- factor(subtype_alpha_month$month,levels = c("0.25","0.5","0.75","1","2","3","6","12","18","36"))
subtype_alpha_month$Term <- as.factor(subtype_alpha_month$Term)
subtype_alpha_month$Study <- as.factor(subtype_alpha_month$Study)
anova(lmerTest::lmer(Richness ~ Term+(1|month)+(1|Study)+(1|st_subjectID),data =subtype_alpha_month))
anova(lmerTest::lmer(Shannon ~ Term+(1|month)+(1|Study)+(1|st_subjectID),data =subtype_alpha_month))
#Figure 2b
preterm_full_richness <- ggplot(subtype_alpha, aes(x=Term, y=Richness)) +
   ...
   scale_x_discrete(labels=c("preterm"="Preterm","full_term"="Term"))+
...
term_preterm_subtype_pval <- data.frame("subtype"="x","term_mean"=0,"term_se"=0,"term_prevalence"=0,"term_median"=0,
                                        "preterm_mean"=0,"preterm_se"=0,"preterm_prevalence"=0,"preterm_median"=0,
                                        "pval_wil"=0,"pval_lm"=0,stringsAsFactors = F)[-1,]
term_subtype_sum_0.05 <- term_subtype_sum%>%filter(prevalence>0.05)
preterm_subtype_sum_0.05 <- preterm_subtype_sum%>%filter(prevalence>0.05)
subtype_list <- c(term_subtype_sum_0.05$subtype,
                  preterm_subtype_sum_0.05$subtype[-which(preterm_subtype_sum_0.05$subtype%in%term_subtype_sum_0.05$subtype)])#510
count=0
for (t in subtype_list) {
   # t="beta_lactam__MIR-12"
   count=count+1
   print(count)
   ...
   db_cb$Study <- as.factor(db_cb$Study)
   db_cb$group <- as.factor(db_cb$group)
   pval_wil <- pvalue(wilcox_test(subtype ~ group|Study, data=db_cb))
   lm_fit <- lmerTest::lmer(subtype ~ group+(1|Study)+(1|st_subjectID),data = db_cb)
   pval_lm <- anova(lm_fit)
   term_preterm_subtype_pval[nrow(term_preterm_subtype_pval)+1,] <- c(t,mean(db_term$subtype),std.error(db_term$subtype),sum(db_term$subtype>0)/1619,median(db_term$subtype),
                                                                      mean(db_preterm$subtype),std.error(db_preterm$subtype),sum(db_preterm$subtype>0)/5684,median(db_preterm$subtype),
                                                                      pval_wil,pval_lm$`Pr(>F)`)
}
term_preterm_subtype_pval_new <- term_preterm_subtype_pval%>%mutate_at(vars(-subtype),as.numeric)
term_preterm_subtype_pval_new$fdr <- p.adjust(term_preterm_subtype_pval_new$pval_lm,method = "fdr")
term_preterm_subtype_pval_new$logfdr <- -log10(term_preterm_subtype_pval_new$fdr)
term_preterm_subtype_pval_new$logfold <- log2(term_preterm_subtype_pval_new$preterm_mean/term_preterm_subtype_pval_new$term_mean)
term_preterm_subtype_pval_new_sig <- term_preterm_subtype_pval_new%>%filter(fdr<0.05)
...
term_preterm_subtype_pval_new$type <- sapply(strsplit(term_preterm_subtype_pval_new$subtype, split='__', fixed=TRUE), function(x)(x[1]))
term_preterm_subtype_pval_new$type[term_preterm_subtype_pval_new$type=="macrolide-lincosamide-streptogramin"] <- "MLS"
term_preterm_subtype_pval_new$type[term_preterm_subtype_pval_new$type=="other_peptide_antibiotics"] <- "OPA"
term_preterm_subtype_pval_new$type[term_preterm_subtype_pval_new$type=="antibacterial_fatty_acid"] <- "AFA"
term_preterm_subtype_pval_new$gene <- sapply(strsplit(term_preterm_subtype_pval_new$subtype, split='__', fixed=TRUE), function(x)(x[2]))
term_preterm_subtype_pval_new <- term_preterm_subtype_pval_new%>%
   ...
   mutate("shape"=case_when(sig=="Up" ~ "Up",
                            sig=="Down" ~ "Down",
                            sig=="Nosig" ~ "NA"))
term_preterm_subtype_pval_new$sig <- factor(term_preterm_subtype_pval_new$sig,levels = c("Nosig","Up","Down"))
term_preterm_subtype_pval_new$type2 <- factor(term_preterm_subtype_pval_new$type2,levels = c(sort(unique(term_preterm_subtype_pval_new$type2))[-9],"NA"))
#Figure 2c
term_preterm_subtype_sig_p <- ggplot() +
   geom_point(data=term_preterm_subtype_pval_new, aes(x=logfold,y=logfdr,size=prevalence,shape=sig,color=type2),stroke=2,alpha=1)+
   scale_shape_manual(values = c(16,16,10))+
   scale_color_manual(values = c("#684795","#386CB0","#7FC97F","#43760A","#FCCDE5","#E6AB02","#F54646","#DA732D","#BC80BD",
                                 "#934B07","#BEAED4","#1B9E77","#E7298A","#F3A2CD","#9A9898"))+
...
#amrfinderplus
##############################################################################################################################
amr_term_profile_coverm <- read.csv("data/amr_term_resistome_coverm_gtdb_gunc_stat_plasmid.csv")
#ABO
amr_term_profile_coverm_gene <- amr_term_profile_coverm%>%group_by(sample,Element_symbol)%>%dplyr::summarise(rpkm_total=sum(rpkm))
amr_term_profile_coverm_gene_sum <- amr_term_profile_coverm_gene%>%
   ...
   arrange(desc(prevalence))
amr_term_profile_coverm_gene_sum <- amr_term_profile_coverm_gene%>%
   group_by(sample,Element_symbol)%>%spread(.,sample,rpkm_total)
amr_term_profile_coverm_gene_sum[is.na(amr_term_profile_coverm_gene_sum)] <- 0
amr_term_profile_coverm_gene_sum_t <- amr_term_profile_coverm_gene_sum%>%column_to_rownames("Element_symbol")%>%t()%>%as.data.frame()
#the richness is based on the amr result regardless of the abundance reported by coverm
amr_term_richness <- amr_term_profile_coverm_gene%>%group_by(sample)%>%dplyr::summarise(n=n())
amr_term_shannon <- diversity(amr_term_profile_coverm_gene_sum_t, index = "shannon", MARGIN=1) 
names(amr_term_richness) <- c("SequenceID","Richness")
amr_term_shannon_df <- amr_term_shannon%>%as.data.frame()%>%rownames_to_column("SequenceID")
names(amr_term_shannon_df)[2] <- "Shannon"
amr_term_alpha <- left_join(amr_term_richness,amr_term_shannon_df,by="SequenceID")
amr_preterm_profile_coverm <- read.csv("data/amr_preterm_resistome_coverm_bins_gtdb_stat_plasmid.csv")
amr_preterm_profile_coverm_gene <- amr_preterm_profile_coverm%>%group_by(sample,Element_symbol)%>%dplyr::summarise(rpkm_total=sum(rpkm))
amr_preterm_profile_coverm_gene_sum <- amr_preterm_profile_coverm_gene%>%
   group_by(sample,Element_symbol)%>%spread(.,sample,rpkm_total)
...
#the richness is based on the amr result regardless of the abundance reported by coverm
amr_preterm_richness <- amr_preterm_profile_coverm_gene%>%group_by(sample)%>%dplyr::summarise(n=n())
amr_preterm_shannon <- diversity(amr_preterm_profile_coverm_gene_sum_t, index = "shannon", MARGIN=1) 
names(amr_preterm_richness) <- c("SequenceID","Richness")
amr_preterm_shannon_df <- amr_preterm_shannon%>%as.data.frame()%>%rownames_to_column("SequenceID")
names(amr_preterm_shannon_df)[2] <- "Shannon"
amr_preterm_alpha <- left_join(amr_preterm_richness,amr_preterm_shannon_df,by="SequenceID")
amr_alpha <- rbind(amr_preterm_alpha,amr_term_alpha)
amr_alpha <- left_join(amr_alpha,metadata_cb,by="SequenceID")
amr_alpha%>%group_by(Term)%>%dplyr::summarise(mean=mean(Richness),median=median(Richness),IQR_25=quantile(Richness)[2],IQR_75=quantile(Richness)[4])
amr_alpha%>%group_by(Term)%>%dplyr::summarise(mean=mean(Shannon),median=median(Shannon),IQR_25=quantile(Shannon)[2],IQR_75=quantile(Shannon)[4])
amr_alpha$Study <- as.factor(amr_alpha$Study)
amr_alpha$month <- as.factor(amr_alpha$month)
amr_alpha$Term <- factor(amr_alpha$Term,levels = c("preterm","full_term"))
anova(lmerTest::lmer(Richness ~ Term+(1|Study)+(1|st_subjectID),data =amr_alpha))
anova(lmerTest::lmer(Shannon ~ Term+(1|Study)+(1|st_subjectID),data =amr_alpha))
amr_alpha_month <- amr_alpha%>%filter(!is.na(DOL))
amr_alpha_month$month <- factor(amr_alpha_month$month,levels = c("0.25","0.5","0.75","1","2","3","6","12","18","36"))
amr_alpha_month$Term <- as.factor(amr_alpha_month$Term)
...
preterm_full_richness_amr <- ggplot(amr_alpha, aes(x=Term, y=Richness)) +
   ...
   scale_fill_manual(values = c("#4994C4","#E09137"))+
   scale_x_discrete(labels=c("preterm"="Preterm","full_term"="Term"))+
   labs(x ="", y="Richness (number of aARG)")+
   theme_bw()+
   theme(panel.grid = element_blank(),axis.line = element_blank(),
         panel.border = element_rect(colour = "black",linewidth = 1))+
   guides(fill=guide_legend(ncol=5))+
...
preterm_full_shannon_amr <- ggplot(amr_alpha, aes(x=Term, y=Shannon)) +
   ...
   scale_fill_manual(values = c("#4994C4","#E09137"))+
   scale_x_discrete(labels=c("preterm"="Preterm","full_term"="Term"))+
   labs(x ="", y="Shannon index (based on aARG)")+
   theme_bw()+
   theme(panel.grid = element_blank(),axis.line = element_blank(),
         panel.border = element_rect(colour = "black",linewidth = 1))+
   guides(fill=guide_legend(ncol=5))+
...
amr_t <- amr_t[which(rowSums(amr_t)!=0),which(colSums(amr_t)!=0)]
amr_t_beta_dist <- vegdist(amr_t, method = "bray")

amr_t_beta_dist_db <- amr_t_beta_dist%>%as.matrix()%>%as.data.frame()
amr_t_beta_dist_db[upper.tri(amr_t_beta_dist_db, diag=T)] <- NA
amr_t_beta_dist_db <- amr_t_beta_dist_db%>%rownames_to_column("run1")
amr_t_beta_dist_db_g <- gather(amr_t_beta_dist_db,run2,distance,-run1)
amr_t_beta_dist_db_g_2 <- amr_t_beta_dist_db_g%>%filter(!is.na(distance))
amr_t_beta_dist_db_g_2 <- left_join(amr_t_beta_dist_db_g_2,metadata_cb%>%dplyr::select(SequenceID,Term),by=c("run1"="SequenceID"))
amr_t_beta_dist_db_g_2 <- left_join(amr_t_beta_dist_db_g_2,metadata_cb%>%dplyr::select(SequenceID,Term),by=c("run2"="SequenceID"))
...
amr_distance <- ggplot(amr_t_beta_dist_db_g_2_sel, aes(x=Term.x, y=distance, fill=Term.x)) +
   ...
   scale_fill_manual(values = c("#4994C4","#E09137"))+
   scale_x_discrete(labels=c("preterm"="Preterm","full_term"="Term"))+
   labs(x ="", y="Bray-Curtis distance based on assembled ARGs")+
   theme_bw()+
   theme(panel.grid = element_blank(),axis.line = element_blank(),
         panel.border = element_rect(colour = "black",linewidth = 1))+
   guides(fill=guide_legend(ncol=5))+
...
metadata_cb_sel <- metadata_cb%>%filter(SequenceID%in%rownames(amr_t))%>%arrange(match(SequenceID,rownames(amr_t)))
metadata_cb_sel$Term <- as.factor(metadata_cb_sel$Term)
set.seed(1000)
with(metadata_cb_sel,vegan::adonis2(amr_t_beta_dist ~ Term,data=metadata_cb_sel,permutations = 1000,strata = Study))
amr_beta_pco <- cmdscale(amr_t_beta_dist, k=2, eig = T)
...
amr_beta_pco.tabble <- right_join(metadata_cb, amr_beta_pco.point, by="SequenceID")
...
amr_term_profile_coverm_2 <- amr_term_profile_coverm %>%
...
amr_term_profile_coverm_2_sum <- amr_term_profile_coverm_2%>%group_by(sample,drug)%>%dplyr::summarise(rpkm_total=sum(rpkm))
amr_term_profile_coverm_3 <- amr_term_profile_coverm_2_sum%>%
   group_by(drug)%>%
   dplyr::summarise("prevalence"=n()/length(unique(.$sample)),
                    "Mean_rpkm"=sum(rpkm_total)/length(unique(.$sample)))%>%
   arrange(desc(prevalence))

amr_term_profile_coverm_2_sum_s <- spread(amr_term_profile_coverm_2_sum,sample,rpkm_total)
amr_term_profile_coverm_2_sum_s[is.na(amr_term_profile_coverm_2_sum_s)] <- 0
term_amr_se <- data.frame("drug"="x","se"=0,stringsAsFactors = F)[-1,]
for (t in amr_term_profile_coverm_2_sum_s$drug) {
   a <- amr_term_profile_coverm_2_sum_s%>%filter(drug==t)%>%column_to_rownames("drug")%>%t()%>%as.data.frame()%>%setNames("drug")
   term_amr_se[nrow(term_amr_se)+1,] <- c(t,std.error(a$drug))
}
amr_term_profile_coverm_3_se <- left_join(amr_term_profile_coverm_3,term_amr_se,by="drug")
amr_term_profile_coverm_3_se$se <- as.numeric(amr_term_profile_coverm_3_se$se)
amr_term_profile_coverm_3_se$drug <- tolower(amr_term_profile_coverm_3_se$drug)
amr_term_profile_coverm_3_se$drug[amr_term_profile_coverm_3_se$drug=="streptogramin b"] <- "streptogramin B"
amr_term_profile_coverm_3_se$drug <- factor(amr_term_profile_coverm_3_se$drug,levels = amr_term_profile_coverm_3_se$drug)
...
amr_term_drug_class_p <- ggplot() +
   ...
   geom_line(data=amr_term_profile_coverm_3_se, aes(x=drug, y=prevalence/0.000008,group=1),linetype = "dashed",color="#E09137",size=0.5)+
   geom_point(data=amr_term_profile_coverm_3_se, aes(x=drug, y=prevalence/0.000008),size=1.5)+
   geom_hline(yintercept=0.9/0.000008,linetype="dashed",size=0.5,color="black")+
   scale_y_continuous(expand = c(0,0),name= "Mean ARG family abundance (rpkm)",limits = c(0,130000),
                      breaks = c(0,30000,60000,90000,120000),labels = c("0","30,000","60,000","90,000","332,327"),
                      sec.axis = sec_axis(~ . *0.000008, breaks = c(0,0.25,0.5,0.75,0.9,1)))+
   theme_bw()+
   theme(panel.grid = element_blank(),axis.line = element_blank(),
         panel.border = element_rect(colour = "black",linewidth = 1))+
   theme(panel.background = element_blank(),
...
amr_preterm_profile_coverm_2 <- amr_preterm_profile_coverm %>%
   dplyr::select(run_contig,sample,rpkm,Subclass)%>%
   dplyr::bind_cols(split_into_multiple(.$Subclass, "/", "cat"))%>%
   gather(.,cat,drug,-c(run_contig,sample,rpkm,Subclass))%>%
   filter(!is.na(drug))%>%
   dplyr::select(-cat)
amr_preterm_profile_coverm_2_sum <- amr_preterm_profile_coverm_2%>%group_by(sample,drug)%>%dplyr::summarise(rpkm_total=sum(rpkm))
amr_preterm_profile_coverm_3 <- amr_preterm_profile_coverm_2_sum%>%
   group_by(drug)%>%dplyr::summarise("Mean_rpkm"=sum(rpkm_total)/length(unique(.$sample)))
amr_preterm_profile_coverm_2_sum_s <- spread(amr_preterm_profile_coverm_2_sum,sample,rpkm_total)
amr_preterm_profile_coverm_2_sum_s[is.na(amr_preterm_profile_coverm_2_sum_s)] <- 0
preterm_amr_se <- data.frame("drug"="x","se"=0,stringsAsFactors = F)[-1,]
for (t in amr_preterm_profile_coverm_2_sum_s$drug) {
   a <- amr_preterm_profile_coverm_2_sum_s%>%filter(drug==t)%>%column_to_rownames("drug")%>%t()%>%as.data.frame()%>%setNames("drug")
   preterm_amr_se[nrow(preterm_amr_se)+1,] <- c(t,std.error(a$drug))
}
amr_preterm_profile_coverm_3_se <- left_join(amr_preterm_profile_coverm_3,preterm_amr_se,by="drug")
amr_preterm_profile_coverm_3_se$se <- as.numeric(amr_preterm_profile_coverm_3_se$se)
amr_preterm_profile_coverm_3_se$drug <- tolower(amr_preterm_profile_coverm_3_se$drug)
amr_preterm_profile_coverm_3_se$drug[amr_preterm_profile_coverm_3_se$drug=="streptogramin b"] <- "streptogramin B"
amr_preterm_profile_coverm_3_se$drug[amr_preterm_profile_coverm_3_se$drug=="streptogramin a"] <- "streptogramin A"
amr_preterm_profile_coverm_3_se$drug[amr_preterm_profile_coverm_3_se$drug=="virginiamycin m"] <- "virginiamycin M"
term_preterm_amr_pval <- data.frame("drug"="x","term_mean"=0,"term_se"=0,"term_prevalence"=0,"term_median"=0,
                                    "preterm_mean"=0,"preterm_se"=0,"preterm_prevalence"=0,"preterm_median"=0,
                                    "pval_wil"=0,"pval_lm"=0,stringsAsFactors = F)[-1,]
amr_class_list <- unique(amr_preterm_profile_coverm_2_sum$drug)
count=0
for (t in amr_class_list) {
   count=count+1
   print(count)
   db_term <- amr_term_profile_coverm_2_sum_s%>%filter(drug==t)%>%column_to_rownames("drug")%>%t()%>%as.data.frame()%>%rownames_to_column("SequenceID")
   if (ncol(db_term)==1) {
      db_term$drug <- 0
      print(t)
   }
   db_term$group <- "term"
   names(db_term)[2] <- "drug"
   db_preterm <- amr_preterm_profile_coverm_2_sum_s%>%filter(drug==t)%>%column_to_rownames("drug")%>%t()%>%as.data.frame()%>%rownames_to_column("SequenceID")
   db_preterm$group <- "preterm"
   names(db_preterm)[2] <- "drug"
   db_cb <- rbind.fill(db_term,db_preterm)
   db_cb <- left_join(db_cb,metadata_cb%>%dplyr::select(SequenceID,Study,st_subjectID),by="SequenceID")
   db_cb$Study <- as.factor(db_cb$Study)
   db_cb$group <- as.factor(db_cb$group)
   pval <- pvalue(wilcox_test(drug ~ group|Study, data=db_cb))
   pval_wil <- pvalue(wilcox_test(drug ~ group|Study, data=db_cb))
   lm_fit <- lmerTest::lmer(drug ~ group+(1|Study)+(1|st_subjectID),data = db_cb)
   pval_lm <- anova(lm_fit)
   term_preterm_amr_pval[nrow(term_preterm_amr_pval)+1,] <- c(t,mean(db_term$drug),std.error(db_term$drug),sum(db_term$drug>0)/1585,median(db_term$drug),
                                                              mean(db_preterm$drug),std.error(db_preterm$drug),sum(db_preterm$drug>0)/5619,median(db_preterm$drug),
                                                              pval_wil,pval_lm$`Pr(>F)`)
}
term_preterm_amr_pval <- term_preterm_amr_pval%>%mutate_at(vars(-drug),as.numeric)
term_preterm_amr_pval$fdr <- p.adjust(term_preterm_amr_pval$pval_lm,method = "fdr")
term_preterm_amr_pval_sel <- term_preterm_amr_pval%>%filter(fdr<0.05)%>%arrange(desc(preterm_mean))
term_preterm_amr_pval_sel$drug <- tolower(term_preterm_amr_pval_sel$drug)
term_preterm_amr_sum <- rbind(amr_term_profile_coverm_3_se%>%mutate(group="full_term")%>%dplyr::select(drug,Mean_rpkm,se,group)%>%filter(drug%in%term_preterm_amr_pval_sel$drug),
                              amr_preterm_profile_coverm_3_se%>%mutate(group="preterm")%>%filter(drug%in%term_preterm_amr_pval_sel$drug))
term_preterm_amr_sum$drug <- as.character(term_preterm_amr_sum$drug)
term_preterm_amr_sum$drug <- factor(term_preterm_amr_sum$drug,levels = term_preterm_amr_pval_sel$drug)
term_preterm_amr_sum$group <- factor(term_preterm_amr_sum$group,levels = c("preterm","full_term"))
...
term_preterm_amr_p <- ggplot(term_preterm_amr_sum, aes(x=drug, y=Mean_rpkm,fill=group)) +
   ...
   scale_fill_manual(values = c("#4994C4","#E09137"),labels=c("preterm"="Preterm","full_term"="Term"))+
   scale_y_continuous(expand = c(0,0),name= "Mean abundance (rpkm)",limits = c(0,160000),
                      breaks = c(0,50000,100000,150000),labels = c("0","50,000","100,000","332,327"))+
   theme_bw()+
   theme(panel.grid = element_blank(),axis.line = element_blank(),
         panel.border = element_rect(colour = "black",linewidth = 1))+
   theme(panel.background = element_blank(),
...
####################################################################################################################################
std.error <- function(x) sd(x)/sqrt(length(x))
amr_term_profile <- read.csv("data/amr_term_resistome_coverm_gtdb_gunc_stat_plasmid.csv")
amr_term_profile$bin[is.na(amr_term_profile$bin)] <- "nobin" 
amr_term_profile_nobin <- amr_term_profile%>%filter(bin=="nobin")
amr_preterm_profile <- read.csv("data/amr_preterm_resistome_coverm_bins_gtdb_stat_plasmid.csv")
plasmid_term_preterm <- rbind(amr_preterm_profile%>%dplyr::select(plasmid)%>%mutate(group="preterm"),
                              amr_term_profile%>%dplyr::select(plasmid)%>%mutate(group="term"))
chisq.test(plasmid_term_preterm$plasmid,plasmid_term_preterm$group,correct = T)

amr_term_profile$domain <- sapply(strsplit(amr_term_profile$GTDB_classification, split=';', fixed=TRUE), function(x)(x[1]))
amr_term_profile$phylum <- sapply(strsplit(amr_term_profile$GTDB_classification, split=';', fixed=TRUE), function(x)(x[2]))
amr_term_profile$class <- sapply(strsplit(amr_term_profile$GTDB_classification, split=';', fixed=TRUE), function(x)(x[3]))
amr_term_profile$order <- sapply(strsplit(amr_term_profile$GTDB_classification, split=';', fixed=TRUE), function(x)(x[4]))
amr_term_profile$family <- sapply(strsplit(amr_term_profile$GTDB_classification, split=';', fixed=TRUE), function(x)(x[5]))
amr_term_profile$genus <- sapply(strsplit(amr_term_profile$GTDB_classification, split=';', fixed=TRUE), function(x)(x[6]))
amr_term_profile$species <- sapply(strsplit(amr_term_profile$GTDB_classification, split=';', fixed=TRUE), function(x)(x[7]))
amr_term_profile$domain[is.na(amr_term_profile$domain)] <- "unknown"
amr_term_profile$phylum[is.na(amr_term_profile$phylum)] <- "unknown"
amr_term_profile$class[is.na(amr_term_profile$class)] <- "unknown"
amr_term_profile$order[is.na(amr_term_profile$order)] <- "unknown"
amr_term_profile$family[is.na(amr_term_profile$family)] <- "unknown"
amr_term_profile$genus[is.na(amr_term_profile$genus)] <- "unknown"
amr_term_profile$species[amr_term_profile$species=="s__"] <- "unknown"
amr_term_profile$species[is.na(amr_term_profile$species)] <- "unknown"
amr_term_profile_species <- amr_term_profile%>%filter(bin!="nobin")
amr_term_profile_domain <- amr_term_profile%>%group_by(domain)%>%dplyr::summarise(n=n())
tax_number <- data.frame("tax"="x","n_tax"=0,"n_args"=0,stringsAsFactors = F)[-1,]
tax_number[1,] <- c("Phylum",length(table(amr_term_profile$phylum))-1,sum(table(amr_term_profile$phylum))-nrow(amr_term_profile%>%filter(phylum=="unknown")))
tax_number[2,] <- c("Class",length(table(amr_term_profile$class))-1,sum(table(amr_term_profile$class))-nrow(amr_term_profile%>%filter(class=="unknown")))
tax_number[3,] <- c("Order",length(table(amr_term_profile$order))-1,sum(table(amr_term_profile$order))-nrow(amr_term_profile%>%filter(order=="unknown")))
tax_number[4,] <- c("Family",length(table(amr_term_profile$family))-1,sum(table(amr_term_profile$family))-nrow(amr_term_profile%>%filter(family=="unknown")))
tax_number[5,] <- c("Genus",length(table(amr_term_profile$genus))-1,sum(table(amr_term_profile$genus))-nrow(amr_term_profile%>%filter(genus=="unknown")))
tax_number[6,] <- c("Species",length(table(amr_term_profile$species))-1,sum(table(amr_term_profile$species))-nrow(amr_term_profile%>%filter(species=="unknown")))
...
for (t in tax) {
   #t="domain"
   a <- amr_term_profile%>%dplyr::group_by(get(t))%>%dplyr::summarise(n=n())%>%arrange(desc(n))
   print(sum(a$n))
   b <- a[c(1:11),] 
   b$propor <- b$n/sum(a$n)
   b[nrow(b)+1,] <- list("Others",nrow(amr_term_profile)-sum(b$n,na.rm = T),1-sum(b$propor,na.rm = T))
   b$taxonomy <- t
...
sum(tax_diver_genus$n)/4455#0.6967452 ARGs from the 10 genera in full-term infants
...
sum(tax_diver_preterm_genus$n)/37118 #0.8424753 ARGs from the 10 genera in preterm infants
...
tax_diver_cb_plot <- ggplot(tax_diver_cb, aes(x=propor, y=name2, fill=group)) +
   ...
   scale_fill_manual(values=c("#4994C4","#E09137"))+
   labs(x ="Proportion (%)", y="")+
   scale_x_continuous(expand = c(0,0),limits = c(-6,13),position = "top",)+
   theme_bw()+
   theme(plot.margin = unit(c(1,5,1,1), "mm"))+
...
amr_taxa_all <- ggplot(tax_diver_2, aes(x=taxonomy, y=n, fill=new_t)) + 
   ...
   theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ #remove the grid
   scale_y_continuous(expand = c(0,0),breaks=c(0,5000,10000,15000),
...
#only species
amr_term_profile_species <- amr_term_profile%>%dplyr::select(sample,species,rpkm)
amr_term_profile_species$species <- gsub("s__","",amr_term_profile_species$species,fixed = T)
amr_term_profile_species_sum <- amr_term_profile_species%>%group_by(sample,species)%>%dplyr::summarise(total=sum(rpkm))
amr_term_profile_species_sum_1 <- amr_term_profile_species_sum%>%
   group_by(species)%>%
   dplyr::summarise("prevalence"=n()/length(unique(.$sample)),
                    "Mean_rpkm"=sum(total)/length(unique(.$sample)))%>%
   arrange(desc(prevalence))
amr_term_profile_species_sum_s <- spread(amr_term_profile_species_sum,sample,total)
amr_term_profile_species_sum_s[is.na(amr_term_profile_species_sum_s)] <- 0
amr_term_species_se <- data.frame("species"="x","se"=0,stringsAsFactors = F)[-1,]
for (t in amr_term_profile_species_sum_s$species) {
   a <- amr_term_profile_species_sum_s%>%filter(species==t)%>%column_to_rownames("species")%>%t()%>%as.data.frame()%>%setNames("species")
   amr_term_species_se[nrow(amr_term_species_se)+1,] <- c(t,std.error(a$species))
}
amr_term_profile_species_sum_2 <- left_join(amr_term_profile_species_sum_1,amr_term_species_se,by="species")
amr_term_profile_species_sum_2$se <- as.numeric(amr_term_profile_species_sum_2$se)
amr_term_profile_species_sum_2 <- amr_term_profile_species_sum_2%>%filter(species!="unknown")
amr_term_profile_species_sum_3 <- amr_term_profile_species_sum_2%>%dplyr::filter(prevalence>0.005)
#get the number of ARG ORFs for each species
amr_term_profile_species_2 <- amr_term_profile_species%>%group_by(species)%>%dplyr::summarise(n_args=n())%>%
   filter(species%in%amr_term_profile_species_sum_3$species)%>%arrange(desc(n_args))
...
amr_taxa_species <- ggplot() +
   ...
   geom_line(data=amr_term_profile_species_sum_3, aes(x=species, y=prevalence*150000,group=1),linetype = "dashed",color="#C43A0A",size=1)+
   geom_point(data=amr_term_profile_species_sum_3, aes(x=species, y=prevalence*150000),size=1)+
   geom_text(data=amr_term_profile_species_sum_3,aes(x=species, y=Mean_rpkm+se,label=label),angle=90,hjust=-0.1,vjust=0.5,size=3)+
   scale_y_continuous(expand = c(0,0),name= "Mean abundance (rpkm)",limits = c(0,25000),
                      breaks = c(0,5000,10000,15000,20000),labels = c("0","5,000","10,000","15,000","20,000"),
                      sec.axis = sec_axis(~ . /150000, breaks = c(0,0.05,0.1,0.15)))+
   theme_bw()+
   theme(panel.grid = element_blank(),axis.line = element_blank(),
         panel.border = element_rect(colour = "black",linewidth = 1))+
   theme(legend.position = "none",
...
amr_term_profile_yes_orf <- amr_term_profile%>%filter(bin!="nobin")%>%filter(plasmid=="Yes")
amr_term_profile_yes_orf$sample_orf <- paste0(amr_term_profile_yes_orf$sample,"_",amr_term_profile_yes_orf$Gene)
amr_cluster_plas <- read.delim("data/amr_term_plasmid_nr_cluster.txt")
amr_cluster_plas$clstr_iden <- gsub("%","",amr_cluster_plas$clstr_iden)
amr_cluster_plas$clstr_cov <- gsub("%","",amr_cluster_plas$clstr_cov)
...
amr_term_profile_yes_orf_cluster <- left_join(amr_term_profile_yes_orf,amr_cluster_plas%>%dplyr::select(id,clstr),by=c("sample_orf"="id"))
for (v in c("phylum","class","order","family","genus","species")) {
   print(v)
   cluster_sum_1 <- amr_term_profile_yes_orf_cluster%>%group_by(clstr)%>%dplyr::summarise(n_taxa_all=length(unique(get(v))),n_args=n())
   cluster_sum_2 <- amr_term_profile_yes_orf_cluster%>%filter(get(v)!="unknown")%>%group_by(clstr)%>%dplyr::summarise(n_taxa_sel=length(unique(get(v))),n_args_2=n())
...
cluster_sum_sel_species <- amr_term_profile_yes_orf_cluster%>%filter(clstr%in%cluster_sum_sel$clstr)
...
plasmid_cluster <- Heatmap(as.matrix(cluster_sum_sel_species_s),cluster_rows=T,cluster_columns=T,col = col_fun_type,
                           ...
                           width = unit(16, "cm"), height = unit(5, "cm"),
...
amr_preterm_profile <- read.csv("data/amr_preterm_resistome_coverm_bins_gtdb_stat_plasmid.csv")
amr_preterm_profile$species <- sapply(strsplit(amr_preterm_profile$GTDB_classification, split=';', fixed=TRUE), function(x)(x[7]))
amr_preterm_profile$species[is.na(amr_preterm_profile$species)] <- "unknown"
amr_preterm_profile$species[amr_preterm_profile$species=="s__"] <- "unknown"
amr_preterm_profile$species <- gsub("s__","",amr_preterm_profile$species,fixed = T)
amr_preterm_profile$ncbi_species <- sapply(strsplit(amr_preterm_profile$NCBI_classification, split=';', fixed=TRUE), function(x)(x[7]))
amr_preterm_profile$ncbi_species[is.na(amr_preterm_profile$ncbi_species)] <- "unknown"
amr_preterm_profile$ncbi_species[amr_preterm_profile$ncbi_species=="s__"] <- "unknown"
amr_preterm_profile$ncbi_species <- gsub("s__","",amr_preterm_profile$ncbi_species,fixed = T)
amr_preterm_profile_species <- amr_preterm_profile%>%dplyr::select(sample,species,rpkm)
amr_preterm_profile_species_sum <- amr_preterm_profile_species%>%group_by(sample,species)%>%dplyr::summarise(total=sum(rpkm))
length(unique(amr_preterm_profile_species_sum$species))#477 species with unknown
amr_preterm_profile_species_sum_1 <- amr_preterm_profile_species_sum%>%
   group_by(species)%>%
   dplyr::summarise("prevalence"=n()/length(unique(.$sample)),
                    "Mean_rpkm"=sum(total)/length(unique(.$sample)))%>%
...
amr_term_profile$species[is.na(amr_term_profile$species)] <- "unknown"
amr_term_profile$species[amr_term_profile$species=="s__"] <- "unknown"
amr_term_profile$species <- gsub("s__","",amr_term_profile$species,fixed = T)
amr_term_profile$ncbi_species <- sapply(strsplit(amr_term_profile$NCBI_classification, split=';', fixed=TRUE), function(x)(x[7]))
amr_term_profile$ncbi_species[is.na(amr_term_profile$ncbi_species)] <- "unknown"
amr_term_profile$ncbi_species[amr_term_profile$ncbi_species=="s__"] <- "unknown"
amr_term_profile$ncbi_species <- gsub("s__","",amr_term_profile$ncbi_species,fixed = T)
amr_term_profile_species <- amr_term_profile%>%dplyr::select(sample,species,rpkm)
amr_term_profile_species_sum <- amr_term_profile_species%>%group_by(sample,species)%>%dplyr::summarise(total=sum(rpkm))
length(unique(amr_term_profile_species_sum$species))#327 species with unknown
amr_term_profile_species_sum_1 <- amr_term_profile_species_sum%>%
   group_by(species)%>%
   dplyr::summarise("prevalence"=n()/length(unique(.$sample)),
                    "Mean_rpkm"=sum(total)/length(unique(.$sample)))%>%
...
shared_list <- data.frame("species"=intersect(amr_term_profile_species_sum_1$species,amr_preterm_profile_species_sum_1$species))
shared_list$genus <- sapply(strsplit(shared_list$species, split=' ', fixed=TRUE), function(x)(x[1]))
#amr class
amr_preterm_profile_drug <- amr_preterm_profile %>%
   dplyr::select(sample,run_contig,species,rpkm,Subclass)%>%
   dplyr::bind_cols(split_into_multiple(.$Subclass, "/", "cat"))%>%
   gather(.,cat,drug,-c(sample,run_contig,species,rpkm,Subclass))%>%
   filter(!is.na(drug))%>%
   dplyr::select(-cat)%>%filter(species!="unknown")

amr_term_profile_drug <- amr_term_profile %>%
   dplyr::select(sample,run_contig,species,rpkm,Subclass)%>%
   dplyr::bind_cols(split_into_multiple(.$Subclass, "/", "cat"))%>%
   gather(.,cat,drug,-c(sample,run_contig,species,rpkm,Subclass))%>%
   filter(!is.na(drug))%>%
...
preterm_term_drug_pval <- data.frame("species"="x","preterm_median"=0,"preterm_mean"=0,"term_median"=0,"term_mean"=0,"pval_wil"=0,"pval_lm"=0,stringsAsFactors = F)[-1,]
count=0
for (s in shared_list$species) {
   # s="Enterococcus faecalis"
   count=count+1
   print(count)
   a_preterm_1 <- amr_preterm_profile_drug%>%filter(species==s)%>%group_by(sample)%>%
      dplyr::summarise(total=sum(rpkm))%>%mutate(group="preterm")
   a_preterm_2 <- data.frame("sample"=unique(amr_preterm_profile$sample),"total"=0,"group"="preterm")%>%filter(!sample%in%a_preterm_1$sample)
   a_preterm <- rbind(a_preterm_1,a_preterm_2)
   a_term_1 <- amr_term_profile_drug%>%filter(species==s)%>%group_by(sample)%>%
      dplyr::summarise(total=sum(rpkm))%>%mutate(group="term")
   a_term_2 <- data.frame("sample"=unique(amr_term_profile$sample),"total"=0,"group"="term")%>%filter(!sample%in%a_term_1$sample)
   a_term <- rbind(a_term_1,a_term_2)
   a <- rbind(a_preterm,a_term)
   a <- left_join(a,metadata_cb%>%dplyr::select(Study,SequenceID,st_subjectID),by=c("sample"="SequenceID"))
   a_sum <- a%>%group_by(group)%>%dplyr::summarise(mean=mean(total),median=median(total))
   a_1 <- a%>%group_by(Study)%>%dplyr::summarise(n=n())%>%filter(n>=2)
   a_2 <- a%>%filter(Study%in%a_1$Study)
   a_2$Study <- as.factor(a_2$Study)
   a_2$group <- as.factor(a_2$group)
   a_pval <- pvalue(wilcox_test(total ~ group|Study,data=a_2))
   lm_fit <- lmerTest::lmer(total ~ group+(1|Study)+(1|st_subjectID),data = a_2)
   pval_lm <- anova(lm_fit)
   preterm_term_drug_pval[nrow(preterm_term_drug_pval)+1,] <- c(s,a_sum$median[1],a_sum$mean[1],a_sum$median[2],a_sum$mean[2],a_pval,pval_lm$`Pr(>F)`)
}
preterm_term_drug_pval <- preterm_term_drug_pval%>%mutate_at(vars(-species),as.numeric)
preterm_term_drug_pval$fdr <- p.adjust(preterm_term_drug_pval$pval_lm,method = "fdr")
preterm_term_drug_pval$diff <- preterm_term_drug_pval$preterm_mean-preterm_term_drug_pval$term_mean

preterm_term_drug_species_pval <- data.frame("species"="x","drug"="x","preterm_total"=0,"term_total"=0,"pval_wil"=0,"pval_lm"=0,stringsAsFactors = F)[-1,]
count=0
for (s in shared_list$species) {
   # s="Bifidobacterium longum"
   count=count+1
   print(count)
   a_preterm_1 <- amr_preterm_profile_drug%>%filter(species==s)%>%group_by(sample,drug)%>%
      dplyr::summarise(total=sum(rpkm))%>%mutate(group="preterm")
   a_term_1 <- amr_term_profile_drug%>%filter(species==s)%>%group_by(sample,drug)%>%
      dplyr::summarise(total=sum(rpkm))%>%mutate(group="term")
   ...
   for (d in unique(a_1$drug)) {
      ...
      a_preterm_2 <- data.frame("sample"=unique(amr_preterm_profile$sample),"drug"=NA,"total"=0,"group"="preterm")%>%filter(!sample%in%b$sample)
      a_term_2 <- data.frame("sample"=unique(amr_term_profile$sample),"drug"=NA,"total"=0,"group"="term")%>%filter(!sample%in%b$sample)
      a_2 <- rbind(a_preterm_2,a_term_2)
      a <- rbind(b,a_2)
      a <- left_join(a,metadata_cb%>%dplyr::select(Study,SequenceID,st_subjectID),by=c("sample"="SequenceID"))
      a_sel <- a%>%group_by(Study)%>%dplyr::summarise(n=n())%>%filter(n>=2)
      a_3 <- a%>%filter(Study%in%a_sel$Study)
      a_3$Study <- as.factor(a_3$Study)
      a_3$group <- as.factor(a_3$group)
      a_pval <- pvalue(wilcox_test(total ~ group|Study,data=a_3))
      lm_fit <- try(lmerTest::lmer(total ~ group+(1|Study)+(1|st_subjectID),data = a_3))
      if (class(lm_fit) == "try-error") {
         if (length(unique(b$group))==2) {
            preterm_term_drug_species_pval[nrow(preterm_term_drug_species_pval)+1,] <- c(s,d,b_sum$total[b_sum$group=="preterm"],b_sum$total[b_sum$group=="term"],a_pval,NA)
         }else if (unique(b$group)=="term") {
            preterm_term_drug_species_pval[nrow(preterm_term_drug_species_pval)+1,] <- c(s,d,0,b_sum$total[b_sum$group=="term"],a_pval,NA)
         }else if (unique(b$group)=="preterm") {
            preterm_term_drug_species_pval[nrow(preterm_term_drug_species_pval)+1,] <- c(s,d,b_sum$total[b_sum$group=="preterm"],0,a_pval,NA)
         }
      }else{
         pval_lm <- anova(lm_fit)
         if (length(unique(b$group))==2) {
            preterm_term_drug_species_pval[nrow(preterm_term_drug_species_pval)+1,] <- c(s,d,b_sum$total[b_sum$group=="preterm"],b_sum$total[b_sum$group=="term"],a_pval,pval_lm$`Pr(>F)`)
         }else if (unique(b$group)=="term") {
            preterm_term_drug_species_pval[nrow(preterm_term_drug_species_pval)+1,] <- c(s,d,0,b_sum$total[b_sum$group=="term"],a_pval,pval_lm$`Pr(>F)`)
         }else if (unique(b$group)=="preterm") {
            preterm_term_drug_species_pval[nrow(preterm_term_drug_species_pval)+1,] <- c(s,d,b_sum$total[b_sum$group=="preterm"],0,a_pval,pval_lm$`Pr(>F)`)
         }
      }
   }
}
preterm_term_drug_species_pval <- preterm_term_drug_species_pval%>%mutate_at(vars(-species,-drug),as.numeric)
preterm_term_drug_species_pval$fdr <- p.adjust(preterm_term_drug_species_pval$pval_lm,method = "fdr")
preterm_term_drug_species_pval$preterm_mean <- preterm_term_drug_species_pval$preterm_total/5619
preterm_term_drug_species_pval$term_mean <- preterm_term_drug_species_pval$term_total/1585
preterm_term_drug_species_pval$diff <- preterm_term_drug_species_pval$preterm_mean-preterm_term_drug_species_pval$term_mean
preterm_term_drug_species_pval_sig <- preterm_term_drug_species_pval%>%filter(pval_wil<0.05)%>%arrange(desc(diff))
#to make plot
preterm_drug <- amr_preterm_profile_drug%>%filter(species%in%shared_list$species)%>%group_by(species,drug)%>%
   dplyr::summarise(total=sum(rpkm))%>%mutate(group=paste0(species,"__preterm"))
term_drug <- amr_term_profile_drug%>%filter(species%in%shared_list$species)%>%group_by(species,drug)%>%
...
preterm_term_drug_spread_2[seq(1,52,2),] <- preterm_term_drug_spread_2[seq(1,52,2),]/5619
preterm_term_drug_spread_2[seq(2,52,2),] <- preterm_term_drug_spread_2[seq(2,52,2),]/1585
...
names(preterm_term_drug_spread_3_log) <- tolower(names(preterm_term_drug_spread_3_log))
names(preterm_term_drug_spread_3_log)[names(preterm_term_drug_spread_3_log)=="streptogramin b"] <- "streptogramin B"
...
term_preterm_species_drug_plus_4 <- data.frame(rep(0,26),rep(0,26),rep(0,26),rep(0,26),rep(0,26),rep(0,26),rep(0,26),rep(0,26),
                                               rep(0,26),rep(0,26),rep(0,26),rep(0,26),rep(0,26),rep(0,26),rep(0,26),rep(0,26),
                                               rep(0,26),rep(0,26),rep(0,26),rep(0,26),rep(0,26),rep(0,26),rep(0,26))#based on the number row of term_preterm_species_drug_plus_3
...
names(term_preterm_species_drug_plus_term_preterm) <- tolower(names(term_preterm_species_drug_plus_term_preterm))
names(term_preterm_species_drug_plus_term_preterm)[names(term_preterm_species_drug_plus_term_preterm)=="streptogramin b"] <- "streptogramin B"
...
row_annotation_left <- 
   rowAnnotation(
      `Gestational age` = rep(c("Preterm","Term"),26),
      col=list(`Gestational age`=c("Preterm"="#4994C4","Term"="#E09137")),width=unit(0.1, 'inches'),
...
metaphlan4_term <- read.csv("data/metaphlan4_term_new.csv")
metaphlan4_taxa <- full_join(metaphlan4_preterm,metaphlan4_term,by="clade_name")
metaphlan4_taxa[is.na(metaphlan4_taxa)] <- 0
metaphlan4_taxa_2 <- metaphlan4_taxa%>%dplyr::select(clade_name,unique(amr_preterm_profile$sample),unique(amr_term_profile$sample))
metaphlan4_taxa_2_kingdom <- metaphlan4_taxa_2%>%filter(grepl("k__",clade_name))%>%filter(!grepl("p__",clade_name))
...
metaphlan4_species <- metaphlan4_species[-which(rowSums(metaphlan4_species)==0),]
#compare ncbi and gtdb species: so the species from gtdb match uniquely to species from ncbi
amr_preterm_profile_term <- rbind(amr_preterm_profile,amr_term_profile)
cp_ncbi_gtdb <- amr_preterm_profile_term%>%group_by(species)%>%dplyr::summarise(n=length(unique(ncbi_species)))
cp_ncbi_gtdb_species <- amr_preterm_profile_term%>%distinct(species,.keep_all = T)%>%dplyr::select(species,ncbi_species)
preterm_drug_2 <- amr_preterm_profile_drug%>%filter(species%in%shared_list$species)%>%group_by(sample,species)%>%dplyr::summarise(drug_total=sum(rpkm))
term_drug_2 <- amr_term_profile_drug%>%filter(species%in%shared_list$species)%>%group_by(sample,species)%>%dplyr::summarise(drug_total=sum(rpkm))
...
shared_list_ncbi$ncbi_species_2 <- gsub("]","",shared_list_ncbi$ncbi_species_2,fixed = T)
which(!shared_list_ncbi$species%in%names(metaphlan4_species))#7 12 13 14 19 20 21 25
which(!shared_list_ncbi$ncbi_species%in%names(metaphlan4_species))#7 12 14
which(!shared_list_ncbi$ncbi_species_2%in%names(metaphlan4_species))#12 14
#keep the species that are present in gtdb and metaphlan4
metaphlan4_shared_list <- metaphlan4_species%>%dplyr::select(shared_list_ncbi$ncbi_species_2[-c(12,14)])
...
#Streptococcus sp001556435 or Streptococcus salivarius (from gtdb, and classified as Streptococcus salivarius in ncbi) are not present in each sample at the same time
strep <- term_drug_2_species%>%filter(ncbi_species=="Streptococcus salivarius")%>%group_by(sample)%>%dplyr::summarise(n=length(unique(species)))
...
term_drug_2_species_sum$mean_drug <- term_drug_2_species_sum$total_drug/1585#1585 samples for amr
term_drug_2_species_sum$mean_abund <- term_drug_2_species_sum$total_abund/1585
term_drug_2_species_sum$group <- "Term"
cor(term_drug_2_species_sum$mean_drug[-c(17,25)],term_drug_2_species_sum$mean_abund[-c(17,25)])
#0.9649076
trendline_sum(term_drug_2_species_sum$mean_drug[-c(17,25)], term_drug_2_species_sum$mean_abund[-c(17,25)], model="line2P",Pvalue.corrected = FALSE,summary = TRUE,eDigit = 5)
...
#Streptococcus sp001556435 or Streptococcus salivarius (from gtdb, and classified as Streptococcus salivarius in ncbi) are not present in each sample at the same time
strep <- preterm_drug_2_species%>%filter(ncbi_species=="Streptococcus salivarius")%>%group_by(sample)%>%dplyr::summarise(n=length(unique(species)))
preterm_drug_2_species_sum <- preterm_drug_2_species%>%ungroup()%>%dplyr::select(-sample)%>%group_by(species)%>%
   dplyr::summarise(total_drug=sum(drug_total),total_abund=sum(species_abund))
preterm_drug_2_species_sum$mean_drug <- preterm_drug_2_species_sum$total_drug/5619#5619 samples for amr
preterm_drug_2_species_sum$mean_abund <- preterm_drug_2_species_sum$total_abund/5619
preterm_drug_2_species_sum$group <- "Preterm"

cor(preterm_drug_2_species_sum$mean_drug[-c(17,25)],preterm_drug_2_species_sum$mean_abund[-c(17,25)])
#0.9588691
trendline_sum(preterm_drug_2_species_sum$mean_drug[-c(17,25)], preterm_drug_2_species_sum$mean_abund[-c(17,25)], model="line2P",Pvalue.corrected = FALSE,summary = TRUE,eDigit = 5)
...
preterm_term_amr_species <- ggplot(preterm_drug_2_species_sum_term, aes(x=mean_drug, y=mean_abund,color=group)) + 
   ...
   scale_color_manual(values = c("#4994C4","#E09137"))+
   labs(x ="Mean abundance of ARG drug classes (rpkm)", y="Mean relative abundance of species (%)")+
   scale_x_continuous(labels=c("0","25,000","50,000","75,000","100,000"))+
   theme_bw()+
   theme(plot.margin = unit(c(1,5,1,1), "mm"))+
   theme(panel.grid = element_blank(),axis.line = element_blank(),
         panel.border = element_rect(colour = "black",linewidth = 1))+
...
#procrustes for species and arg genes
preterm_taxa_sel_2 <- metaphlan4_taxa%>%dplyr::select(clade_name,unique(amr_preterm_profile$sample))
preterm_taxa_kingdom <- preterm_taxa_sel_2%>%filter(grepl("k__",clade_name))%>%filter(!grepl("p__",clade_name))
preterm_taxa_kingdom$clade_name#"k__Bacteria"  "k__Eukaryota" "k__Archaea" 
...
preterm_taxa_species_2 <- preterm_taxa_species_2%>%dplyr::select(-clade_name)%>%column_to_rownames("species")%>%
   t()%>%as.data.frame()%>%dplyr::select(which(colSums(.)!=0))
preterm_taxa_species_2 <- preterm_taxa_species_2[-which(rowSums(preterm_taxa_species_2)==0),]

amr_preterm_gene_sum <- amr_preterm_profile%>%group_by(sample,Element_symbol)%>%dplyr::summarise(rpkm=sum(rpkm))%>%spread(.,Element_symbol,rpkm)
amr_preterm_gene_sum[is.na(amr_preterm_gene_sum)] <- 0
amr_preterm_gene_sum_sel <- amr_preterm_gene_sum%>%dplyr::select(sample,which(colSums(.[-1])!=0)+1)%>%filter(sample%in%rownames(preterm_taxa_species_2))
amr_preterm_gene_sum_sel <- amr_preterm_gene_sum_sel[which(rowSums(amr_preterm_gene_sum_sel[-1])!=0),]

preterm_taxa_species_3 <- preterm_taxa_species_2%>%filter(rownames(.)%in%amr_preterm_gene_sum_sel$sample)
amr_preterm_gene_sum_1 <- amr_preterm_gene_sum_sel%>%arrange(match(sample,rownames(preterm_taxa_species_3)))%>%column_to_rownames("sample")
...
preterm_gene_h <- decostand(amr_preterm_gene_sum_1,method = "hellinger")
...
term_taxa_sel_2 <- metaphlan4_taxa%>%dplyr::select(clade_name,unique(amr_term_profile$sample))
term_taxa_kingdom <- term_taxa_sel_2%>%filter(grepl("k__",clade_name))%>%filter(!grepl("p__",clade_name))
term_taxa_kingdom$clade_name#"k__Bacteria"  "k__Eukaryota" "k__Archaea" 
...
term_taxa_species_2 <- term_taxa_species_2%>%dplyr::select(-clade_name)%>%column_to_rownames("species")%>%
   t()%>%as.data.frame()%>%dplyr::select(which(colSums(.)!=0))
amr_term_gene_sum <- amr_term_profile%>%group_by(sample,Element_symbol)%>%dplyr::summarise(rpkm=sum(rpkm))%>%spread(.,Element_symbol,rpkm)
amr_term_gene_sum[is.na(amr_term_gene_sum)] <- 0
amr_term_gene_sum_sel <- amr_term_gene_sum%>%dplyr::select(sample,which(colSums(.[-1])!=0)+1)
term_taxa_species_3 <- term_taxa_species_2%>%filter(rownames(.)%in%amr_term_gene_sum_sel$sample)
amr_term_gene_sum_1 <- amr_term_gene_sum_sel%>%arrange(match(sample,rownames(term_taxa_species_3)))%>%column_to_rownames("sample")
...
term_gene_h <- decostand(amr_term_gene_sum_1,method = "hellinger")
...
term_preterm_species <- term_preterm_species[-which(rowSums(term_preterm_species)==0),]

term_preterm_species_dist <- vegdist(term_preterm_species, method = "bray")
metadata_cb_order <- metadata_cb%>%filter(SequenceID%in%rownames(term_preterm_species))%>%arrange(match(SequenceID,rownames(term_preterm_species)))
identical(metadata_cb_order$SequenceID,rownames(term_preterm_species))
set.seed(1000)
with(metadata_cb_order,vegan::adonis2(term_preterm_species_dist ~ Term,data=metadata_cb_order,permutations = 1000,strata = Study))
...
term_preterm_species_sum_1 <- term_preterm_species%>%rownames_to_column("SequenceID")%>%
   left_join(.,metadata_cb%>%dplyr::select(SequenceID,Term),by="SequenceID")
...
term_preterm_species_sum_total_n$perterm_prev <- term_preterm_species_sum_total_n$preterm_n/5677
term_preterm_species_sum_total_n$term_prev <- term_preterm_species_sum_total_n$term_n/1619
term_preterm_species_sum_total_n_sel <- term_preterm_species_sum_total_n%>%filter(species%in%shared_list_ncbi$ncbi_species_2)

preterm_term_species_pval <- data.frame("species"="x","preterm_median"=0,"preterm_mean"=0,"term_median"=0,"term_mean"=0,"pval_wil"=0,"pval_lm"=0,stringsAsFactors = F)[-1,]
count=0
for (s in term_preterm_species_sum_total_n_sel$species) {
   ...
   a_meta <- left_join(a,metadata_cb%>%dplyr::select(SequenceID,Term,Study,st_subjectID),by="SequenceID")
   names(a_meta)[2] <- "taxa"
   a_meta$Study <- as.factor(a_meta$Study)
   a_meta$Term <- as.factor(a_meta$Term)
   a_meta_sum <- a_meta%>%group_by(Term)%>%dplyr::summarise(mean=mean(taxa),median=median(taxa))
   pval <- pvalue(wilcox_test(taxa ~ Term | Study, data = a_meta))
   lm_fit <- lmerTest::lmer(taxa ~ Term+(1|Study)+(1|st_subjectID),data = a_meta)
   pval_lm <- anova(lm_fit)
   preterm_term_species_pval[nrow(preterm_term_species_pval)+1,] <- c(s,a_meta_sum$median[2],a_meta_sum$mean[2],a_meta_sum$median[1],a_meta_sum$mean[1],pval,pval_lm$`Pr(>F)`)  
}
preterm_term_species_pval <- preterm_term_species_pval%>%mutate_at(vars(-species),as.numeric)
preterm_term_species_pval$fdr <- p.adjust(preterm_term_species_pval$pval_lm,method = "fdr")
preterm_term_species_pval$diff <- preterm_term_species_pval$preterm_mean-preterm_term_species_pval$term_mean
preterm_term_species_pval_sig <- preterm_term_species_pval%>%filter(pval_lm<0.05)%>%arrange(desc(diff))
