...
term_type_p <- ggplot() +
   ...
   scale_y_continuous(expand = c(0,0),name= "Mean ARG abundance (capc)",limits = c(0,1),
...
preterm_full_shannonn_rgi <- ggplot(rgi_alpha, aes(x=Term, y=Shannon)) +
   geom_violin(aes(fill=Term),alpha=1,trim=T)+
   geom_boxplot(width=0.1,outlier.size = 1)+
   scale_fill_manual(values = c("#4994C4","#E09137"))+
   scale_x_discrete(labels=c("preterm"="Preterm","full_term"="Full term"))+
...
rgi_t_beta_dist_db <- rgi_t_beta_dist%>%as.matrix()%>%as.data.frame()
rgi_t_beta_dist_db[upper.tri(rgi_t_beta_dist_db, diag=T)] <- NA
rgi_t_beta_dist_db <- rgi_t_beta_dist_db%>%rownames_to_column("run1")
rgi_t_beta_dist_db_g <- gather(rgi_t_beta_dist_db,run2,distance,-run1)
rgi_t_beta_dist_db_g_2 <- rgi_t_beta_dist_db_g%>%filter(!is.na(distance))
rgi_t_beta_dist_db_g_2 <- left_join(rgi_t_beta_dist_db_g_2,metadata_cb%>%dplyr::select(SequenceID,Term),by=c("run1"="SequenceID"))
rgi_t_beta_dist_db_g_2 <- left_join(rgi_t_beta_dist_db_g_2,metadata_cb%>%dplyr::select(SequenceID,Term),by=c("run2"="SequenceID"))
rgi_t_beta_dist_db_g_2_sel <- rgi_t_beta_dist_db_g_2%>%filter(Term.x==Term.y)
rgi_t_beta_dist_db_g_2_sel$Term.x <- factor(rgi_t_beta_dist_db_g_2_sel$Term.x,levels = c("preterm","full_term"))
wilcox_test(distance ~ Term.x,data = rgi_t_beta_dist_db_g_2_sel)
#Supplementary Figure 6e
rgi_distance <- ggplot(rgi_t_beta_dist_db_g_2_sel, aes(x=Term.x, y=distance, fill=Term.x)) +
   geom_boxplot(outlier.size = 1)+
   stat_summary(fun="mean")+
   scale_fill_manual(values = c("#4994C4","#E09137"))+
   scale_x_discrete(labels=c("preterm"="Preterm","full_term"="Full term"))+
   labs(x ="", y="Bray-Curtis distance based on ARG families")+
   theme_bw()+
   theme(panel.grid = element_blank(),axis.line = element_blank(),
         panel.border = element_rect(colour = "black",linewidth = 1))+
   guides(fill=guide_legend(ncol=5))+
...
#differential rgi drugs between preterm and term infants
rgi_preterm_profile_coverm_2 <- rgi_preterm_profile_coverm %>%
   dplyr::select(cb,sample,rpkm,Drug.Class)%>%
   dplyr::bind_cols(split_into_multiple(.$Drug.Class, "; ", "cat"))%>%
   gather(.,cat,drug,-c(cb,sample,rpkm,Drug.Class))%>%
   filter(!is.na(drug))%>%
   dplyr::select(-cat)
rgi_preterm_profile_coverm_2_sum <- rgi_preterm_profile_coverm_2%>%group_by(sample,drug)%>%dplyr::summarise(rpkm_total=sum(rpkm))
rgi_preterm_profile_coverm_3 <- rgi_preterm_profile_coverm_2_sum%>%
   group_by(drug)%>%dplyr::summarise("Mean_rpkm"=sum(rpkm_total)/length(unique(.$sample)))

rgi_preterm_profile_coverm_2_sum_s <- spread(rgi_preterm_profile_coverm_2_sum,sample,rpkm_total)
rgi_preterm_profile_coverm_2_sum_s[is.na(rgi_preterm_profile_coverm_2_sum_s)] <- 0
preterm_rgi_se <- data.frame("drug"="x","se"=0,stringsAsFactors = F)[-1,]
for (t in rgi_preterm_profile_coverm_2_sum_s$drug) {
   a <- rgi_preterm_profile_coverm_2_sum_s%>%filter(drug==t)%>%column_to_rownames("drug")%>%t()%>%as.data.frame()%>%setNames("drug")
   preterm_rgi_se[nrow(preterm_rgi_se)+1,] <- c(t,std.error(a$drug))
}
rgi_preterm_profile_coverm_3_se <- left_join(rgi_preterm_profile_coverm_3,preterm_rgi_se,by="drug")
rgi_preterm_profile_coverm_3_se$se <- as.numeric(rgi_preterm_profile_coverm_3_se$se)
rgi_preterm_profile_coverm_3_se$drug <- gsub(" antibiotic","",rgi_preterm_profile_coverm_3_se$drug,fixed = T)
rgi_preterm_profile_coverm_3_se$drug[rgi_preterm_profile_coverm_3_se$drug=="antibacterial free fatty acids"] <- "AFFA"
rgi_preterm_profile_coverm_3_se$drug[rgi_preterm_profile_coverm_3_se$drug=="disinfecting agents and antiseptics"] <- "DAA"

term_preterm_rgi_pval <- data.frame("drug"="x","term_mean"=0,"term_prevalence"=0,"preterm_mean"=0,"preterm_prevalence"=0,"pval"=0,stringsAsFactors = F)[-1,]
rgi_class_list <- unique(rgi_preterm_profile_coverm_2_sum$drug)
count=0
for (t in rgi_class_list) {
   # t="tetracycline antibiotic"
   count=count+1
   print(count)
   db_term <- rgi_term_profile_coverm_2_sum_s%>%filter(drug==t)%>%column_to_rownames("drug")%>%t()%>%as.data.frame()%>%rownames_to_column("SequenceID")
   db_term$group <- "term"
   names(db_term)[2] <- "drug"
   db_preterm <- rgi_preterm_profile_coverm_2_sum_s%>%filter(drug==t)%>%column_to_rownames("drug")%>%t()%>%as.data.frame()%>%rownames_to_column("SequenceID")
   db_preterm$group <- "preterm"
   ...
   term_preterm_rgi_pval[nrow(term_preterm_rgi_pval)+1,] <- c(t,mean(db_term$drug),sum(db_term$drug>0)/1355,mean(db_preterm$drug),sum(db_preterm$drug>0)/3300,pval)
}
term_preterm_rgi_pval <- term_preterm_rgi_pval%>%mutate_at(vars(-drug),as.numeric)
term_preterm_rgi_pval$fdr <- p.adjust(term_preterm_rgi_pval$pval,method = "fdr")
term_preterm_rgi_pval_sig <- term_preterm_rgi_pval%>%filter(fdr<0.05)
term_preterm_rgi_pval_sig$diff <- term_preterm_rgi_pval_sig$preterm_mean-term_preterm_rgi_pval_sig$term_mean

term_preterm_rgi_pval_sel <- term_preterm_rgi_pval%>%filter(fdr<0.05)%>%arrange(desc(preterm_mean))
nrow(term_preterm_rgi_pval_sel)#21
term_preterm_rgi_pval_sel$drug <- gsub(" antibiotic","",term_preterm_rgi_pval_sel$drug,fixed = T)
term_preterm_rgi_pval_sel$drug[term_preterm_rgi_pval_sel$drug=="antibacterial free fatty acids"] <- "AFFA"
term_preterm_rgi_pval_sel$drug[term_preterm_rgi_pval_sel$drug=="disinfecting agents and antiseptics"] <- "DAA"

term_preterm_rgi_sum <- rbind(rgi_term_profile_coverm_3_se%>%mutate(group="full_term")%>%dplyr::select(drug,Mean_rpkm,se,group)%>%filter(drug%in%term_preterm_rgi_pval_sel$drug),
                              rgi_preterm_profile_coverm_3_se%>%mutate(group="preterm")%>%filter(drug%in%term_preterm_rgi_pval_sel$drug))
term_preterm_rgi_sum$drug <- as.character(term_preterm_rgi_sum$drug)
term_preterm_rgi_sum$drug <- factor(term_preterm_rgi_sum$drug,levels = term_preterm_rgi_pval_sel$drug)
term_preterm_rgi_sum$group <- factor(term_preterm_rgi_sum$group,levels = c("preterm","full_term"))
term_preterm_rgi_sum$group2 <- paste0(term_preterm_rgi_sum$group,"_",term_preterm_rgi_sum$drug)
#Supplementary Figure 6d
term_preterm_rgi_p <- ggplot(term_preterm_rgi_sum, aes(x=drug, y=Mean_rpkm,fill=group)) +
   geom_bar(stat="identity",position=position_dodge(),color="black",width = 0.8)+
...
term_preterm_rgi_sum_2 <- term_preterm_rgi_sum%>%filter(drug%in%term_preterm_rgi_pval_sel$drug[21])
#Supplementary Figure 6d
term_preterm_rgi_p_2 <- ggplot(term_preterm_rgi_sum_2, aes(x=drug, y=Mean_rpkm,fill=group)) +
   geom_bar(stat="identity",position=position_dodge(),color="black",width = 0.8)+
   geom_errorbar(aes(ymin=Mean_rpkm, ymax=Mean_rpkm+se),position=position_dodge(0.8),width=0.5)+
   scale_fill_manual(values = c("#4994C4","#E09137"),labels=c("preterm"="Preterm","full_term"="Full term"))+
   scale_y_continuous(expand = c(0,0),limits = c(0,420),name= "Mean abundance (rpkm)")+
   theme_bw()+
   theme(panel.grid = element_blank(),axis.line = element_line(colour = "black"),
         panel.border = element_blank())+
   theme(panel.background = element_blank(),
...
####################################################################################################################################
#Enriched carriage of ARG potentials from bacterial species in preterm infants
####################################################################################################################################
term_metadata <- read.csv("data/metadata_cb_infant_term.csv")
preterm_metadata <- read.csv("data/metadata_cb_infant_preterm.csv")
metadata_cb <- rbind(preterm_metadata,term_metadata)
...
for (s in shared_list$species) {
   ...
   preterm_term_drug_pval[nrow(preterm_term_drug_pval)+1,] <- c(s,a_sum$mean[1],a_sum$mean[2],a_pval)
}
...
length(unique(preterm_term_drug_species_pval_sig$species))#23
length(unique(preterm_term_drug_species_pval_sig$drug))#31
...
names(term_preterm_species_drug_plus_term_preterm)[34]
names(term_preterm_species_drug_plus_term_preterm)[34] <- "AFFA"
...
preterm_term_rgi_species <- ggplot(preterm_drug_2_species_sum_term%>%filter(!is.na(total_abund)), aes(x=mean_drug, y=mean_abund,color=group)) + 
   ...
   theme(panel.background = element_blank(),
         ...
         axis.title.y = element_text(size=12, colour="black"),
         axis.title.x = element_text(size=12, colour = "black"))
ggsave(preterm_term_rgi_species,filename = '~/Desktop/work/preterm/analysis/revision/figure/term/preterm_term_rgi_species_re.pdf',
...
for (s in term_preterm_species_sum_total_n_sel$species) {
   ...
   preterm_term_species_pval[nrow(preterm_term_species_pval)+1,] <- c(s,a_meta_sum$mean[2],a_meta_sum$mean[1],pval)  
}
