...
#loso
auc_loso_cb <- read.csv("data/auc_loso_cb.csv")
auc_loso_cb$group <- paste0(auc_loso_cb$feature,"_",auc_loso_cb$batch)
auc_loso_cb$batch <- as.factor(auc_loso_cb$batch)
wilcox_test(auc ~ batch, data=auc_loso_cb%>%filter(feature=="subtype"))
wilcox_test(auc ~ batch, data=auc_loso_cb%>%filter(feature=="species"))
wilcox_test(auc ~ batch, data=auc_loso_cb%>%filter(feature=="subtype_species"))

auc_loso_cb$feature <- as.factor(auc_loso_cb$feature)
data2=auc_loso_cb%>%filter(feature%in%c("subtype","species"))%>%filter(batch=="original")
wilcox_test(auc ~ feature, data=auc_loso_cb%>%filter(feature%in%c("subtype","species"))%>%filter(batch=="original"))
wilcox_test(auc ~ feature, data=auc_loso_cb%>%filter(feature%in%c("subtype","subtype_species"))%>%filter(batch=="original"))
wilcox_test(auc ~ feature, data=auc_loso_cb%>%filter(feature%in%c("species","subtype_species"))%>%filter(batch=="original"))
wilcox_test(auc ~ feature, data=auc_loso_cb%>%filter(feature%in%c("subtype","species"))%>%filter(batch=="adjust"))
wilcox_test(auc ~ feature, data=auc_loso_cb%>%filter(feature%in%c("subtype","subtype_species"))%>%filter(batch=="adjust"))
wilcox_test(auc ~ feature, data=auc_loso_cb%>%filter(feature%in%c("species","subtype_species"))%>%filter(batch=="adjust"))
auc_loso_cb$batch <- factor(auc_loso_cb$batch,levels = c("original","adjust"))
auc_loso_cb$feature <- factor(auc_loso_cb$feature,levels = c("subtype","species","subtype_species"))
#Supplementary Figure 12b
auc_loso_compa <- ggplot(auc_loso_cb, aes(x=feature,y=auc,fill=feature)) + 
   geom_bar(stat="identity",width=0.6)+
   facet_grid(study~batch,
              labeller = labeller(batch=c("original"="Original","adjust"="Batch adjusted")))+
