...
#richness is based rgi regardless of the abundance from coverm
...
by(rgi_alpha_meta$Richness,rgi_alpha_meta$nec2,summary)
by(rgi_alpha_meta$Shannon,rgi_alpha_meta$nec2,summary)
rgi_alpha_meta$nec2 <- as.factor(rgi_alpha_meta$nec2)
rgi_alpha_meta$Study <- as.factor(rgi_alpha_meta$Study)
wilcox_test(Richness ~ nec2 | Study, data = rgi_alpha_meta%>%filter(nec2%in%c("no","preNEC")))
wilcox_test(Richness ~ nec2 | Study, data = rgi_alpha_meta%>%filter(nec2%in%c("no","postNEC")))
wilcox_test(Richness ~ nec2 | Study, data = rgi_alpha_meta%>%filter(nec2%in%c("preNEC","postNEC")))
wilcox_test(Shannon ~ nec2 | Study, data = rgi_alpha_meta%>%filter(nec2%in%c("no","preNEC")))
wilcox_test(Shannon ~ nec2 | Study, data = rgi_alpha_meta%>%filter(nec2%in%c("no","postNEC")))
wilcox_test(Shannon ~ nec2 | Study, data = rgi_alpha_meta%>%filter(nec2%in%c("preNEC","postNEC")))
rgi_alpha_meta_g <- rgi_alpha_meta%>%dplyr::select(Richness,Shannon,SequenceID,nec2)%>%gather(.,group,value,-SequenceID,-nec2)
rgi_alpha_meta_g$group[rgi_alpha_meta_g$group=="Shannon"] <- "Shannon index"
rgi_alpha_meta_g$nec2 <- factor(rgi_alpha_meta_g$nec2,levels = c("no","preNEC","postNEC"))
#Supplementary Figure 11b
rgi_gene_alpha_p <- ggplot(data=rgi_alpha_meta_g,aes(x=nec2, y=value, col=nec2)) +
   geom_boxplot() + 
...
rgi_t_no_pre <- rgi_preterm_profile_coverm_gene_sum_t%>%filter(rownames(.)%in%metadata_nec_sel_no_pre$SequenceID)
rgi_t_no_pre <- rgi_t_no_pre[which(rowSums(rgi_t_no_pre)!=0),which(colSums(rgi_t_no_pre)!=0)]
rgi_t_no_pre_dist_beta <- vegdist(rgi_t_no_pre, method = "bray")
rgi_t_no_pre_dist_beta_meta <- metadata_nec_sel%>%filter(SequenceID%in%rownames(rgi_t_no_pre))%>%
   arrange(match(SequenceID,rownames(rgi_t_no_pre)))
identical(rownames(rgi_t_no_pre),rgi_t_no_pre_dist_beta_meta$SequenceID)#TRUE
set.seed(1000)
with(rgi_t_no_pre_dist_beta_meta, vegan::adonis2(rgi_t_no_pre_dist_beta ~ nec2,data=rgi_t_no_pre_dist_beta_meta,permutations = 1000, strata = Study))
#no NEC vs. post-NEC
metadata_nec_sel_no_post <- metadata_nec_sel%>%filter(nec2%in%c("no","postNEC"))
rgi_t_no_post <- rgi_preterm_profile_coverm_gene_sum_t%>%filter(rownames(.)%in%metadata_nec_sel_no_post$SequenceID)
rgi_t_no_post <- rgi_t_no_post[which(rowSums(rgi_t_no_post)!=0),which(colSums(rgi_t_no_post)!=0)]
rgi_t_no_post_dist_beta <- vegdist(rgi_t_no_post, method = "bray")
rgi_t_no_post_dist_beta_meta <- metadata_nec_sel%>%filter(SequenceID%in%rownames(rgi_t_no_post))%>%
   arrange(match(SequenceID,rownames(rgi_t_no_post)))
identical(rownames(rgi_t_no_post),rgi_t_no_post_dist_beta_meta$SequenceID)#TRUE
set.seed(1000)
with(rgi_t_no_post_dist_beta_meta, vegan::adonis2(rgi_t_no_post_dist_beta ~ nec2,data=rgi_t_no_post_dist_beta_meta,permutations = 1000, strata = Study))
#pre-NEC vs. post-NEC
metadata_nec_sel_pre_post <- metadata_nec_sel%>%filter(nec2%in%c("preNEC","postNEC"))
rgi_t_pre_post <- rgi_preterm_profile_coverm_gene_sum_t%>%filter(rownames(.)%in%metadata_nec_sel_pre_post$SequenceID)
rgi_t_pre_post <- rgi_t_pre_post[which(rowSums(rgi_t_pre_post)!=0),which(colSums(rgi_t_pre_post)!=0)]
rgi_t_pre_post_dist_beta <- vegdist(rgi_t_pre_post, method = "bray")
rgi_t_pre_post_dist_beta_meta <- metadata_nec_sel%>%filter(SequenceID%in%rownames(rgi_t_pre_post))%>%
   arrange(match(SequenceID,rownames(rgi_t_pre_post)))
identical(rownames(rgi_t_pre_post),rgi_t_pre_post_dist_beta_meta$SequenceID)#TRUE
set.seed(1000)
with(rgi_t_pre_post_dist_beta_meta, vegan::adonis2(rgi_t_pre_post_dist_beta ~ nec2,data=rgi_t_pre_post_dist_beta_meta,permutations = 1000, strata = Study))
...
rgi_t_dist_beta <- vegdist(rgi_t, method = "bray")
rgi_t_dist_beta_meta <- metadata_nec_sel%>%filter(SequenceID%in%rownames(rgi_t))%>%
   arrange(match(SequenceID,rownames(rgi_t)))
identical(rownames(rgi_t),rgi_t_dist_beta_meta$SequenceID)#TRUE
set.seed(1000)
with(rgi_t_dist_beta_meta, vegan::adonis2(rgi_t_dist_beta ~ nec2,data=rgi_t_dist_beta_meta,permutations = 1000, strata = Study))
rgi_beta_pco <- cmdscale(rgi_t_dist_beta, k=2, eig = T)
...
rgi_beta_pco.tabble <- right_join(rgi_t_dist_beta_meta, rgi_beta_pco.point, by="SequenceID")
rgi_beta_df.plot <- tibble(Axis1 = rgi_beta_pco.tabble$V1,
                           Axis2 = rgi_beta_pco.tabble$V2,
                           Sample_ID = rgi_beta_pco.tabble$SubjectID,
                           nec2=rgi_beta_pco.tabble$nec2)
rgi_beta_df.plot$nec2 <- factor(rgi_beta_df.plot$nec2,levels = c("no","preNEC","postNEC"))
#Supplementary Figure 11b
rgi_gene_beta_time_p <- ggplot(data=rgi_beta_df.plot,aes(x=Axis1, y=Axis2, col=nec2)) +
...
by(subtype_alpha_meta$Richness,subtype_alpha_meta$nec2,summary)
by(subtype_alpha_meta$Shannon,subtype_alpha_meta$nec2,summary)
subtype_alpha_meta$nec2 <- as.factor(subtype_alpha_meta$nec2)
subtype_alpha_meta$Study <- as.factor(subtype_alpha_meta$Study)
wilcox_test(Richness ~ nec2 | Study, data = subtype_alpha_meta%>%filter(nec2%in%c("no","preNEC")))
wilcox_test(Richness ~ nec2 | Study, data = subtype_alpha_meta%>%filter(nec2%in%c("no","postNEC")))
wilcox_test(Richness ~ nec2 | Study, data = subtype_alpha_meta%>%filter(nec2%in%c("preNEC","postNEC")))
wilcox_test(Shannon ~ nec2 | Study, data = subtype_alpha_meta%>%filter(nec2%in%c("no","preNEC")))
wilcox_test(Shannon ~ nec2 | Study, data = subtype_alpha_meta%>%filter(nec2%in%c("no","postNEC")))
wilcox_test(Shannon ~ nec2 | Study, data = subtype_alpha_meta%>%filter(nec2%in%c("preNEC","postNEC")))
...
by(species_alpha_meta$Richness,species_alpha_meta$nec2,summary)
by(species_alpha_meta$Shannon,species_alpha_meta$nec2,summary)
species_alpha_meta$nec2 <- as.factor(species_alpha_meta$nec2)
species_alpha_meta$Study <- as.factor(species_alpha_meta$Study)
wilcox_test(Richness ~ nec2 | Study, data = species_alpha_meta%>%filter(nec2%in%c("no","preNEC")))
wilcox_test(Richness ~ nec2 | Study, data = species_alpha_meta%>%filter(nec2%in%c("no","postNEC")))
wilcox_test(Richness ~ nec2 | Study, data = species_alpha_meta%>%filter(nec2%in%c("preNEC","postNEC")))
wilcox_test(Shannon ~ nec2 | Study, data = species_alpha_meta%>%filter(nec2%in%c("no","preNEC")))
wilcox_test(Shannon ~ nec2 | Study, data = species_alpha_meta%>%filter(nec2%in%c("no","postNEC")))
wilcox_test(Shannon ~ nec2 | Study, data = species_alpha_meta%>%filter(nec2%in%c("preNEC","postNEC")))
